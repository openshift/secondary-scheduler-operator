...
// Ginkgo test specs for HA Mode - calls the shared test functions
var _ = g.Describe("[sig-scheduling][Operator][Serial] SecondaryScheduler HA Mode", g.Ordered, func() {
	var (
		ctx        context.Context
	...
	g.It("should toggle between single replica and HA mode", func() {
		testHAModeToggle(g.GinkgoTB(), ctx, kubeClient)
	})
})
