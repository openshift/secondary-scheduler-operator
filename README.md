# README

## FBC catalog rendering

To initiliaze catalog-template.json

```sh
$ opm migrate registry.redhat.io/redhat/redhat-operator-index:v4.17 ./catalog-migrate
$ mkdir -p v4.17/catalog/openshift-secondary-scheduler-operator
$ opm alpha convert-template basic -o yaml ./catalog-migrate/openshift-secondary-scheduler-operator/catalog.json > v4.17/catalog-template.yaml
```

To update the catalog

```
$ opm alpha render-template basic v4.17/catalog-template.yaml --migrate-level bundle-object-to-csv-metadata > v4.17/catalog/openshift-secondary-scheduler-operator/catalog.json
```

## Releases

| osso version | bundle image                                                     |
| ------------ | ---------------------------------------------------------------- |
| 1.2.0        | 775e1822637d308859c4eff42d7b09c949610430ff27cf08d01d41e44cbac2ff |
| 1.2.1        | 959d5567fa57212bb1d707c6cd0b67d47af4431d7b567f81bcb7e19892fdde30 |
| 1.2.2        | fe9c58508028d2b9561e09104e11c04b412b060e2c5bae5cb237e20e91dacff8 |
| 1.2.3        | 41cfb2120ec1613ce78ab5eba98d9447e4ea37681db3c4939842c0ecf6cdb0f7 |
| 1.3.0        | 4cea92798fa738944ec3487604e0974e8403e9b31b6330981cb28f12acafed05 |
| 1.3.1        | 082fec20563feb75173232be85f9cca2d564f7bf141a99973e3bd993ec6f6f33 |
| 1.3.2        | f74547b7a26d7da6c5a81dbf9955b8bc934e46626c3c80cd479d63b4853abc43 |
