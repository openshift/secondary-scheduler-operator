...
for OCP_VERSION in $(find . -maxdepth 1 -type d -name 'v4.*' | sed 's|^\./||' | sort -t. -k1,1V -k2,2nr); do
    ...
    # tools update-fbc-catalog --catalog ${OCP_VERSION}/catalog-template.yaml --version 4.${VERSION_NUM}
    if [ "$VERSION_NUM" -ge 17 ]; then
        ...
        opm alpha render-template basic $OCP_VERSION/catalog-template.yaml > $OCP_VERSION/catalog/openshift-secondary-scheduler-operator/catalog.json
    fi
done
